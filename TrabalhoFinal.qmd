...
precos.energia %>%
  ...
  labs(
    ...
    y = "densidade de probabilidade", 
    title = "Distribuição dos preços da energia no mercado Ibérico entre 2018-2022"
  ) + theme_minimal() + 
  scale_colour_discrete(name="",
                            breaks=c("Dados observados", "Distribuição ajustada"),
                            labels=c("Dados observados", "Distribuição ajustada"))
...
data.frame(tir.vec) %>% 
  ...
  labs(
    x = "TIR", 
    y = "Frequência", 
    title =paste("Distribuição da TIR obtida na simulação de Monte Carlo com n =", n.sim)
  ) + theme_minimal() + 
  scale_colour_discrete(name="",
                            breaks=c("TIR", "CI 95%"),
                            labels=c("TIR", "CI 95%"))
